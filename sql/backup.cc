...
bool run_backup_stage(THD *thd, backup_stages stage)
{
  ...
  if (stage == BACKUP_START)
  {
    if (thd->current_backup_stage == BACKUP_FINISHED)
      thd->current_backup_stage= stage;
    else
    {
  ...
  else if ((uint) thd->current_backup_stage >= (uint) stage)
  {
    my_error(ER_BACKUP_WRONG_STAGE, MYF(0), stage_names[stage],
  ...
  for (;
       (uint) thd->current_backup_stage <= (uint) stage ;
       thd->current_backup_stage= ((backup_stages)
                                   ((uint) thd->current_backup_stage+1)))
  {
    bool res;
    switch (thd->current_backup_stage) {
    case BACKUP_START:
...
static bool backup_flush(THD *thd)
{
  ...
  /*
    ...
    This is done with MDL_BACKUP_STAGE1 in lock_global_read_lock.
  */
...
/**
   ...
   Block commits and writing to the binary log
*/
